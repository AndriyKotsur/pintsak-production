...
export const getTileTypes = () => {
	return async dispatch => {
		dispatch({ type: GET_TILE_TYPES_LOADING})
		try {
			const response = await HTTP.getTypes()
			return dispatch({type: GET_TILE_TYPES_SUCCESS, payload: response})
		} catch (err) {
			return dispatch({type: GET_TILE_TYPES_ERROR})
		}
	}
}

export const addTile = ({
	types, images, url, title, type, width, height, thickness, weight_per_meter,
	pieces_per_meter,is_popular, is_available, grey, yellow, orange, red, brown, black,

}) => {
	return async dispatch => {
		dispatch({ type: ADD_TILE_LOADING})
		try {
			...
			return dispatch({ type: ADD_TILE_SUCCESS })
		} catch (err) {
			return dispatch({ type: ADD_TILE_ERROR })
		}
	}
}
